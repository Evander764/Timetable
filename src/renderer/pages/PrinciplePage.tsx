...
import type { PrincipleDisplayMode } from '@shared/types/app'
...
export function PrinciplePage() {
  ...
  function updateDisplayMode(mode: PrincipleDisplayMode): void {
    void updateData({ type: 'principle/update', payload: { displayMode: mode } })
    void updateWidget({ key: 'principle', changes: { enabled: mode === 'standalone' && principle.enabled } })
  }
      ...
      <div className="grid grid-cols-[1fr_420px] gap-4">
          ...
          <div className="mt-5 grid gap-5">
            ...
            <div className="grid grid-cols-2 gap-4">
              <ToggleRow
                ...
                onChange={(checked) => {
                  void updateWidget({ key: 'principle', changes: { enabled: checked && displayMode === 'standalone' } })
            ...
            <div className="rounded-[20px] border border-slate-200/80 bg-white/88 px-4 py-4">
              <div className="mb-3 text-lg font-semibold text-slate-900">展示形态</div>
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <button
                    key={mode}
                    type="button"
                    className={`rounded-lg border px-3 py-2 text-sm font-medium ${displayMode === mode ? 'border-blue-300 bg-blue-50 text-blue-600' : 'border-slate-200 bg-white text-slate-600'}`}
                    onClick={() => updateDisplayMode(mode as PrincipleDisplayMode)}
                  >
                    {label}
                  </button>
                ))}
            ...
            <div className="rounded-[20px] border border-slate-200/80 bg-white/88 px-4 py-4">
              ...
              />
              {displayMode === 'embedded' ? <div className="mt-3 text-sm text-slate-500">当前为嵌入主卡片模式，位置设置只在“单独显示”时生效。</div> : null}
            </div>
          </div>
        </Card>
        ...
        <Card className="relative overflow-hidden">
          ...
          <div className="mt-6 text-sm text-slate-500">
            当前形态：{displayMode === 'embedded' ? '嵌入主卡片' : '单独显示'} · 位置：{Math.round(widget.x)}, {Math.round(widget.y)} 尺寸：{Math.round(widget.width)} × {Math.round(widget.height)}
          </div>
